...
- `DEFAULT_TOPIC` (~line 25) — Nave's Topics' default, `"JESUS, THE CHRIST"`. Each `DATASET_CONFIG` entry also has its own `defaultTopic` (BSB Topics: `"Light"`, Concordance: `"Eternal"`, Prophecy: `"[All] OT + NT Combined"` — one of the synthetic aggregate topics, see below, Custom Dataset: `"Jesus is God"`). `getDatasetDefaultTopic(mode)` resolves the right one; an empty-string `defaultTopic` (none currently) would return `null` and leave the search field blank, with the custom `#topic-suggestions` listbox (see below) still populated.
- `FALLBACK_NAV_TOPIC` — used if `DEFAULT_TOPIC` can't be resolved.
- Topic keys are UPPERCASE strings in `data/topics-with-references.json` (e.g. `"JESUS, THE CHRIST"`, `"LOVE"`, `"FAITH"`); other datasets use natural-case keys.
...
- `DATASET_CONFIG` (~line 62) switches between five datasets, all sharing one `{name, references: {BOOKID: [{verse, subtopics, refs}]}, books}` schema: **Nave's Topics** (`topics`), **BSB Topics** (`bsb-topics`), **Prophecy** (`prophecy`, has its own aggregate-topic logic via `PROPHECY_AGGREGATE_TOPICS`), **BSB Concordance** (`concordance`, word-level references with a stopword filter), and **Custom Dataset** (`custom`, hand-maintained via `source/custom-topics.json` + `scripts/build-custom-dataset.js`). `allTopicNames` (`Object.keys(topicsData)`) is each dataset's topic list in its JSON file's key order — reused as-is by the random-topic button, the suggestion dropdown, and the Previous/Next topic buttons (see below); reordering a dataset's source JSON changes all three. `switchDataset()` (extracted from the `#dataset-mode` `<select>`'s `change` handler) runs on every dataset switch and tries to carry the current search term over: an exact case-insensitive match in the new dataset first, then the same startsWith/contains matcher `getTopicOptions()` uses for suggestions, and only then that dataset's `defaultTopic` via `getDatasetDefaultTopic()`. See `docs/treemap-and-datasets.md` for dataset schemas, source files, and parser scripts.
- The search field (`#topic-input`) has a circular `&times;` clear button (`#topic-clear-btn`, inside `.topic-input-wrap`) instead of a separate "Reset Topic" button. Clicking it always clears the field to no topic (`applyTopicSelection(null, { commit: true })`), in every state. `updateTopicActionState()` hides it whenever `selectedTopic` is null (nothing to clear). Autocomplete is **not** a native `<datalist>` — it's a custom-built `#topic-suggestions` listbox (`renderTopicSuggestions()` etc.), built because native datalist popups are unreliable on mobile.
- `.topic-search-label-row` also holds four nav buttons after the 🎲 random button: Back/Forward (`#topic-back-btn`/`#topic-forward-btn`) retrace `topicHistoryStack` (`{mode, topic}` pairs, spans dataset switches, pushed by `pushTopicHistory()`), and Previous/Next (`#topic-prev-btn`/`#topic-next-btn`) step through `allTopicNames` in-place. All four disable at their respective boundaries via `updateTopicActionState()`.
- Deep links: `?dataset=<mode>&topic=<name>` in the URL overrides `localStorage` on boot, and `syncUrlState()` keeps the URL in sync via `history.replaceState` whenever a topic is committed (including the exact-match-while-typing case from selecting a suggestion, not just blur/Enter — see `applyTopicSelection`'s `commit || isExactMatch` check).
